...
#    geneinfo = entrez_summary(db="gene", id = entrez)
    chrm_data[entry,]$gene_symbol = select(org.Hs.eg.db, keys = entrez, column="SYMBOL", keytype="ENTREZID", multiVals = "first")$SYMBOL
...
#    if (is(res, "try-error")) { chrm_data[entry,]$nearest_gene = "error in getting gene"}

     print (chrm_data[entry,])

}

write.csv(chrm_data, paste(c(filedest,"/", vir, "_virusint/", vir, "_samples_with_locus_and_genes_closestgenes_nopubs.csv"), collapse=""))
